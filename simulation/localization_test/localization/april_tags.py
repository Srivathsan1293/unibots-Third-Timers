...
def tag_world_from_sim(supervisor):
    """Read the actual spawned AprilTag positions. Call after spawn_tags() and
    after at least one robot.step()."""
    children = supervisor.getRoot().getField("children")
    table = {}
    for i in range(children.getCount()):
        node = children.getMFNode(i)
        if node is None or node.getTypeName() != "AprilTag":
            continue
        name = node.getField("name").getSFString()      # "tag_5"
        try:
